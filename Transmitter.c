...
// kraj inita
 ...
 while(1) {                         // Infinite loop
  ...
  if (Button(&GPIOA_IDR, 6, 1, 1)){                      // detektujemo kada je pritisnuto dugme za prelazak u mod recievera
       GPIOD_ODR = 0;					 // gasimo lampice u bloku
       GPIOD_ODR.HAINT3 = ~GPIOD_ODR.HAINT3;		 // invertujemo jednu u bloku
       zvuk = 0;					 // ne saljemo zvuk
       stanje = 1;					 // pamtimo da smo u modu recievera
    ...
    if (zvuk!=0){                                        // ako pustamo zvuk (transmit mode)
      ...
      // Prosledi signal, jer nismo u modu transmitera ni recievera (sto znaci da smo u modu releja)
      if (DATA_RX[0]==1 || DATA_RX[0]==2) {  
